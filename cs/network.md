...
- 传输层：提供端到端的可靠报文传递和错误恢复（数据段Segment）；TCP、UDP、SPX。
  - TCP：传输控制协议，提供面向连接、可靠、拥塞控制、字节流的数据传输服务，数据单位为报文段。
  - UDP：用户数据报协议，提供无连接、尽最大努力、报文的数据传输服务，数据单位为用户数据报。
...
- 首部长度：占4位，因此最大值为15。值为1表示的是1个32位字的长度，也就是4字节。因为固定部分长度为 20 字节，因此该值最小为 5。如果可选字段的长度不是 4 字节的整数倍，就用尾部的填充部分来填充。
...
不是两次握手的原因：已失效的连接请求报文段又传到了服务器端。其次，两次握手无法保证Client正确接收第二次握手的报文（Server无法确认Client是否收到），也无法保证Client和Server之间成功互换初始序列号。

#### 四次挥手

- Client FIN置为1（销毁连接的意思），发送Client seq给Server，状态为FIN_WAIT_1。
- Server收到Client FIN，将Server ACK置为1，Server ack置为Client seq + 1，状态为CLOSE_WAIT。
- Server FIN置为1，发送Server seq给Client，状态为LAST_ACK。
- Client收到Server FIN，将Client ACK置为1，Client ack置为Server seq + 1，状态为TIME_WAIT，Server检查Client seq（Server seq + 1），状态为CLOSED，Client等待2*MSL（报文段最长寿命）后状态为CLOSED。

不是三次挥手的原因：服务器收到客户端断开连接的请求时，可能还有一些数据没有发完，这时先回复ACK，表示接收到了断开连接的请求。等到数据发完之后再发FIN，断开服务器到客户端的数据传送。
...
- 服务器从中选出一组加密算法与HASH算法，向客户端发送证书链。
- 客户端验证服务器的合法性后
...
- 服务器接收数据后
  - 使用自己的私钥解密，得到对称加密的密钥。
  - 用这个密钥解密出Hash摘要值，并验证握手消息是否一致。
  - 服务器使用对称加密的密钥加密握手消息发给客户端，客户端解密并验证摘要，若一致，则握手结束。
